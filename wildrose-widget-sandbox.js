(() => {
  ...
  root.innerHTML = `
    ...
    <section class="wr-panel" aria-label="${cfg.title}">
      ...
      <main class="wr-view active" data-view="voice"><div class="wr-voice"><div><div class="wr-orb"></div><div class="wr-status">Ready when you are</div><div class="wr-note">Tap start and speak naturally. Rose can qualify the request, summarize it, and hand it off.</div><button class="wr-primary wr-start-voice">Start talking</button><button class="wr-secondary wr-end-voice" style="display:none">End</button><div class="wr-live-transcript"><div class="wr-live-label">Live transcript</div><div class="wr-live-lines"><div class="wr-live-empty">Your voice conversation will appear here and in Chat.</div></div></div></div></div></main>
  ...
  function appendVoiceTranscript(role, text, partial = false) {
    ...
    const live = $(".wr-live-lines");
    live?.querySelector(".wr-live-empty")?.remove();
    const liveLine = document.createElement("div");
    liveLine.className = `wr-live-line ${speaker}`;
    liveLine.textContent = `${speaker === "bot" ? "Rose" : "You"}: ${clean}`;
    live?.appendChild(liveLine);
    if (live) live.scrollTop = live.scrollHeight;
    if (!partial) addMsg(speaker, clean);
  }
