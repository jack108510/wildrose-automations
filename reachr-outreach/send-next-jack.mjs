...
const MESSENGER_STATE_PATH = path.join(ROOT, 'jack-sender-state.json');
...
const REQUIRED_PROFILE_PATH = '/jack.sereda.2025/';
const MANUAL_BASELINE_DATE = '';
const MANUAL_BASELINE_COUNT = 0;
...
export function hasExactApproval(prospect) {
  const approval = prospect?.jackApproval;
...
export async function sendViaMessenger(prospect) {
  ...
  if (!pageId) throw new Error('Jack sender requires a numeric verified Page route.');
  ...
  try {
    ...
    if (profile.result?.result?.value !== REQUIRED_PROFILE_PATH) {
      throw new Error(`Jack sender identity preflight failed: fresh /me resolved to ${profile.result?.result?.value || 'unknown'}.`);
    }
    ...
    if (!senderProof.verified) throw new Error('Jack sender identity verification failed.');
...
async function main() {
  ...
  if (!dryRun && process.env.REACHR_JACK_LIVE !== '1') { console.log('NO_SEND:jack_live_disabled'); return; }
